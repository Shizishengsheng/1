...
bool HitBrick::init() {
	...
	//积分
	_scoreLabel = Label::createWithSystemFont(" ", "fonts/arial.ttf", 32);
	...
	//返回主菜单
	auto label1 = Label::createWithSystemFont("Play", "fonts/arial.ttf", 30);

	auto menuitem = MenuItemLabel::create(label1, CC_CALLBACK_1(HitBrick::EnterGameMenu, this));
...
void HitBrick::onKeyReleased(EventKeyboard::KeyCode code, Event* event) {
	...
	case cocos2d::EventKeyboard::KeyCode::KEY_SPACE:   // 蓄力结束，小球发射
		...
		if (spHolded == 1) {
			...
			ball->getPhysicsBody()->setVelocity(Vec2(0, spFactor * 9));
...
bool HitBrick::onConcactBegin(PhysicsContact & contact) {
	...
	if (c1->getTag() == Tag::BOX && c2->getTag() == Tag::BALL) {
		...
		//分数
		_scoreLabel->setString(StringUtils::format("%i", _scorevalue += 50));
...
void HitBrick::GameOver() {
	...
	auto label4 = Label::createWithTTF("主菜单", "fonts/STXINWEI.TTF", 40);

	label4->setColor(Color3B(0, 0, 0));

	auto menuBtn = MenuItemLabel::create(label4, CC_CALLBACK_1(HitBrick::EnterGameMenu, this));

	Menu* menu = Menu::create(menuBtn, NULL);
...
//返回主菜单
void HitBrick::EnterGameMenu(Ref *pSender)
{
